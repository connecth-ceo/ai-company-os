...
TASK_STATUS_VALUES = ("QUEUED", "DISPATCHED", "RUNNING", "COMPLETED", "FAILED")
APPROVAL_STATUS_VALUES = ("PENDING", "APPROVED", "REJECTED")
REVIEW_VERDICT_VALUES = ("PASS", "REWORK")


def enum_type(values: tuple[str, ...], name: str) -> sa.Enum:
...
def upgrade() -> None:
    bind = op.get_bind()
    task_status = enum_type(TASK_STATUS_VALUES, "taskstatus")
    approval_status = enum_type(APPROVAL_STATUS_VALUES, "approvalstatus")
    review_verdict = enum_type(REVIEW_VERDICT_VALUES, "reviewverdict")
    if bind.dialect.name == "postgresql":
        task_status.create(bind, checkfirst=True)
        approval_status.create(bind, checkfirst=True)
        review_verdict.create(bind, checkfirst=True)
    ...
    op.create_table(
        ...
        sa.Column("verdict", review_verdict, nullable=True),
...
def downgrade() -> None:
    ...
    if bind.dialect.name == "postgresql":
        enum_type(REVIEW_VERDICT_VALUES, "reviewverdict").drop(bind, checkfirst=True)
        enum_type(APPROVAL_STATUS_VALUES, "approvalstatus").drop(bind, checkfirst=True)
        enum_type(TASK_STATUS_VALUES, "taskstatus").drop(bind, checkfirst=True)
    # ### end Alembic commands ###
