...
- 목표·프로젝트 카드에는 현재 상태에서 허용된 다음 전이만 표시한다.
- 달성·취소·완료·보관 같은 종료 전이는 확인 후 API에 기록한다.
- 목표일, 보류 상태, 실패 업무와 완료율을 `portfolio-health-v1` 규칙으로 계산해 요약·배지·진척 막대로 표시한다.
- 건강도 조회는 읽기 전용이며 목표·프로젝트 상태를 자동 변경하지 않는다.
...
- 목표·프로젝트 생성·상태 변경은 회사 내부 데이터 기록이며 자동 실행을 시작하지 않는다.
- AI 팀 화면은 읽기 전용이다.
