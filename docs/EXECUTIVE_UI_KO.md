...
이미 구현된 Goal/Project/Task 관계와 AI Employee Registry를 대표가 한 화면에서 확인하고 사용할 수 있게
한다. 이 단계는 기존 실행 제어면을 바꾸지 않으며 외부 행동이나 유료 AI 호출을 추가하지 않는다.

## 목표·프로젝트 화면

- 목표의 성공 기준, 책임자, 목표일, 연결 프로젝트와 업무 진척을 표시한다.
- CEO Desk에서 목표를 만들고 새 프로젝트의 상위 목표를 선택할 수 있다.
...
- 목표·프로젝트 생성은 회사 내부 데이터 기록이며 자동 실행을 시작하지 않는다.
- AI 팀 화면은 읽기 전용이다.
...
- CEO Desk HTML에 목표·프로젝트 선택·목록·생성 폼과 AI 팀 목록이 존재하는지 검사한다.
